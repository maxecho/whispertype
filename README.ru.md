...
появляется уже распознанное: плашка подгоняется под текст, как пузырёк в
переписке, — растёт вширь, потом переносит строки, но не больше трёх. Обе
штуки выключаются в меню.
...
- **Словарь замен** — «как программа написала = как надо». Самое полезное,
  если в речи много имён и жаргона. Важно: слева пишите ровно то, что увидели
  в готовом тексте, а не то, как слово звучит — `Hype = Hybe`, а не
  `хайб = Hybe`. Срабатывает всегда, регистр первой буквы подставляется сам.
