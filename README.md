...
загруженным, поэтому текст появляется почти сразу (около 1.3 секунды на 5 секунд
речи). Хочется меньше — переключите `model` на вариант `-q4`.
...
и `test_pipeline.py` (расшифровка синтезированной речи, без микрофона):

```bash
.venv/bin/python test_hotkey.py && .venv/bin/python test_pipeline.py
```
